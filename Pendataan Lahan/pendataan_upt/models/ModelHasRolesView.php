 ...
 * Page class
 */
class ModelHasRolesView extends ModelHasRoles
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ModelHasRolesView";
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("role_id") ?? Route("role_id")) !== null) {
                ...
                $this->RecKey["role_id"] = $this->role_id->QueryStringValue;
            } elseif (Post("role_id") !== null) {
                $this->role_id->setFormValue(Post("role_id"));
                $this->RecKey["role_id"] = $this->role_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->role_id->setQueryStringValue($keyValue);
                $this->RecKey["role_id"] = $this->role_id->QueryStringValue;
            } else {
                $returnUrl = "modelhasroleslist"; // Return to list
            }
            if (($keyValue = Get("model_type") ?? Route("model_type")) !== null) {
                $this->model_type->setQueryStringValue($keyValue);
                $this->RecKey["model_type"] = $this->model_type->QueryStringValue;
            } elseif (Post("model_type") !== null) {
                $this->model_type->setFormValue(Post("model_type"));
                $this->RecKey["model_type"] = $this->model_type->FormValue;
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "modelhasroleslist"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "modelhasroleslist"; // Not page request, return to list
        }
        if ($returnUrl != "") {
