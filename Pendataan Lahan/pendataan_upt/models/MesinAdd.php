...
class MesinAdd extends Mesin
{
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("mesinlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "mesinlist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "mesinview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->no_seri->CurrentValue = null;
        $this->no_seri->OldValue = $this->no_seri->CurrentValue;
        $this->nama_penyewa->CurrentValue = null;
        $this->nama_penyewa->OldValue = $this->nama_penyewa->CurrentValue;
        ...
        $this->tanggal_sewa->CurrentValue = null;
        $this->tanggal_sewa->OldValue = $this->tanggal_sewa->CurrentValue;
        $this->tanggal_kembali->CurrentValue = null;
        $this->tanggal_kembali->OldValue = $this->tanggal_kembali->CurrentValue;
        $this->foto->CurrentValue = null;
        $this->foto->OldValue = $this->foto->CurrentValue;
        $this->gambar_mesin->CurrentValue = null;
        $this->gambar_mesin->OldValue = $this->gambar_mesin->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'gambar_mesin' first before field var 'x_gambar_mesin'
        $val = $CurrentForm->hasValue("gambar_mesin") ? $CurrentForm->getValue("gambar_mesin") : $CurrentForm->getValue("x_gambar_mesin");
        if (!$this->gambar_mesin->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->gambar_mesin->Visible = false; // Disable update for API request
            } else {
                $this->gambar_mesin->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->gambar_mesin->CurrentValue = $this->gambar_mesin->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['no_seri'] = $this->no_seri->CurrentValue;
        $row['nama_penyewa'] = $this->nama_penyewa->CurrentValue;
        ...
        $row['tanggal_sewa'] = $this->tanggal_sewa->CurrentValue;
        $row['tanggal_kembali'] = $this->tanggal_kembali->CurrentValue;
        $row['foto'] = $this->foto->CurrentValue;
        $row['gambar_mesin'] = $this->gambar_mesin->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->gambar_mesin->EditValue = HtmlEncode($this->gambar_mesin->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->gambar_mesin->Required) {
            if (!$this->gambar_mesin->IsDetailKey && EmptyValue($this->gambar_mesin->FormValue)) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // no_seri
        $this->no_seri->setDbValueDef($rsnew, $this->no_seri->CurrentValue, "", false);

        // nama_penyewa
        $this->nama_penyewa->setDbValueDef($rsnew, $this->nama_penyewa->CurrentValue, "", false);
        ...
        // tanggal_sewa
        $this->tanggal_sewa->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_sewa->CurrentValue, 0), CurrentDate(), false);

        // tanggal_kembali
        $this->tanggal_kembali->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_kembali->CurrentValue, 0), CurrentDate(), false);

        // foto
        $this->foto->setDbValueDef($rsnew, $this->foto->CurrentValue, null, false);

        // gambar_mesin
        $this->gambar_mesin->setDbValueDef($rsnew, $this->gambar_mesin->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("mesinlist"), "", $this->TableVar, true);
