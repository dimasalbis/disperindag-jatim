...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {
        var ruleInput = this.getRuleInput(rule, i),
            $ruleInput = typeof ruleInput == 'string' ? $($.parseHTML(ruleInput.trim())) : $(ruleInput); //***
  ...
  <div class="rules-group-header"> \
    ...
    <div class="btn-group btn-group-sm group-conditions" role="group"> \
      {{~ it.conditions: condition }} \
        <input type="radio" class="btn-check" name="{{= it.group_id }}_cond" id="{{= it.group_id }}_cond" value="{{! condition }}"> \
        <label class="btn btn-primary" for="{{= it.group_id }}_cond">{{= it.translate("conditions", condition) }}</label> \
      {{~}} \
    </div> \
...
QueryBuilder.define('bt-tooltip-errors', function(options) {
    ...
    this.on('getRuleTemplate.filter getGroupTemplate.filter', function(h) {
        ...
        $h.find(QueryBuilder.selectors.error_container).tooltip(options);
...
QueryBuilder.extend(/** @lends module:plugins.ChangeFilters.prototype */ {
    ...
    setFilters: function(deleteOrphans, filters) {
        ...
        if (this.settings.plugins) {
            ...
            } else if (this.settings.plugins['select2']) { //***
                this.$el.find(QueryBuilder.selectors.rule_filter).trigger('change');
            }
        }
 ...
 * @param {object} [options]
 * @param {string} [options.icon='fa-solid fa-circle-info']
 * @param {string} [options.mode='popover'] - inline or popover
 * @throws ConfigError
 */
    ...
    // POPOVER
        ...
        this.on('afterUpdateRuleFilter afterUpdateRuleOperator', function(e, rule) {
            ...
            if (!description) {
                ...
                if ($b.length === 0) {
                    $b = $($.parseHTML('<button type="button" class="btn btn-info filter-description" data-bs-toggle="popover"><i class="' + options.icon + '"></i></button>'));
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(Selectors.condition_container).after(
                '<button type="button" class="btn btn-default" data-invert="group">' +
        ...
        if (options.display_rules_button && options.invert_rules) {
            this.on('getRuleTemplate.filter', function(h) {
                ...
                $h.find(Selectors.rule_actions).prepend(
                    '<button type="button" class="btn btn-default" data-invert="rule">' +
                    '<i class="' + options.icon + '"></i> ' + self.translate('invert') +
                    '</button>'
                );
                h.value = $h.prop('outerHTML');
 ...
 * @param {object} [options]
 * @param {string} [options.icon_checked='fa-solid fa-square-check']
 * @param {string} [options.icon_unchecked='fa-solid fa-square']
 */
QueryBuilder.define('not-group', function(options) {
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            ...
            $h.find(QueryBuilder.selectors.condition_container).prepend(
                '<button type="button" class="btn btn-default" data-not="group">' +
    ...
    icon_unchecked: 'fa-solid fa-square',
    icon_checked: 'fa-solid fa-square-check',
    disable_template: false
});
...
QueryBuilder.define('select2', function(options) {
    if (!$.fn.select2 || !$.fn.select2.constructor) {
    ...
    this.on('afterCreateRuleFilters', function(e, rule) {
        rule.$el.find(Selectors.rule_filter).select2($.extend({}, options, rule.filter.select2Options ?? {}));
    });
...
QueryBuilder.extend(/** @lends module:plugins.UniqueFilter.prototype */ {
    ...
    applyDisabledFilters: function(e) {
        ...
        } else if (self.settings.plugins['select2']) { //***
            this.$el.find(QueryBuilder.selectors.rule_filter).trigger('change');
        }
    }
});
