 ...
 * Page class
 */
class PersonalAccessTokensList extends PersonalAccessTokens
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PersonalAccessTokensList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpersonal_access_tokenslist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "personalaccesstokensadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "personalaccesstokensdelete";
        $this->MultiUpdateUrl = "personalaccesstokensupdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fpersonal_access_tokenslistsrch";
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server" && isset($UserProfile)) {
            $savedFilterList = $UserProfile->getSearchFilters(CurrentUserName(), "fpersonal_access_tokenslistsrch");
        }
        $filterList = Concat($filterList, $this->id->AdvancedSearch->toJson(), ","); // Field id
        $filterList = Concat($filterList, $this->tokenable_type->AdvancedSearch->toJson(), ","); // Field tokenable_type
        $filterList = Concat($filterList, $this->tokenable_id->AdvancedSearch->toJson(), ","); // Field tokenable_id
        ...
        $filterList = Concat($filterList, $this->abilities->AdvancedSearch->toJson(), ","); // Field abilities
        $filterList = Concat($filterList, $this->last_used_at->AdvancedSearch->toJson(), ","); // Field last_used_at
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fpersonal_access_tokenslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->tokenable_type->AdvancedSearch->SearchValue = @$filter["x_tokenable_type"];
        $this->tokenable_type->AdvancedSearch->SearchOperator = @$filter["z_tokenable_type"];
        $this->tokenable_type->AdvancedSearch->SearchCondition = @$filter["v_tokenable_type"];
        $this->tokenable_type->AdvancedSearch->SearchValue2 = @$filter["y_tokenable_type"];
        $this->tokenable_type->AdvancedSearch->SearchOperator2 = @$filter["w_tokenable_type"];
        $this->tokenable_type->AdvancedSearch->save();

        // Field tokenable_id
        $this->tokenable_id->AdvancedSearch->SearchValue = @$filter["x_tokenable_id"];
        $this->tokenable_id->AdvancedSearch->SearchOperator = @$filter["z_tokenable_id"];
        $this->tokenable_id->AdvancedSearch->SearchCondition = @$filter["v_tokenable_id"];
        $this->tokenable_id->AdvancedSearch->SearchValue2 = @$filter["y_tokenable_id"];
        $this->tokenable_id->AdvancedSearch->SearchOperator2 = @$filter["w_tokenable_id"];
        $this->tokenable_id->AdvancedSearch->save();
        ...
        // Field abilities
        $this->abilities->AdvancedSearch->SearchValue = @$filter["x_abilities"];
        $this->abilities->AdvancedSearch->SearchOperator = @$filter["z_abilities"];
        $this->abilities->AdvancedSearch->SearchCondition = @$filter["v_abilities"];
        $this->abilities->AdvancedSearch->SearchValue2 = @$filter["y_abilities"];
        $this->abilities->AdvancedSearch->SearchOperator2 = @$filter["w_abilities"];
        $this->abilities->AdvancedSearch->save();

        // Field last_used_at
        $this->last_used_at->AdvancedSearch->SearchValue = @$filter["x_last_used_at"];
        $this->last_used_at->AdvancedSearch->SearchOperator = @$filter["z_last_used_at"];
        $this->last_used_at->AdvancedSearch->SearchCondition = @$filter["v_last_used_at"];
        $this->last_used_at->AdvancedSearch->SearchValue2 = @$filter["y_last_used_at"];
        $this->last_used_at->AdvancedSearch->SearchOperator2 = @$filter["w_last_used_at"];
        $this->last_used_at->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->tokenable_type, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->name, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->_token, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->abilities, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-action ew-multi-delete\" title=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" href=\"#\" onclick=\"return ew.submitAction(event, {f:document.fpersonal_access_tokenslist, url:'" . GetUrl($this->MultiDeleteUrl) . "', data:{action:'show'}});return false;\">" . $Language->phrase("DeleteSelectedLink") . "</a>";
        $item->Visible = $Security->canDelete();
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpersonal_access_tokenslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpersonal_access_tokenslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fpersonal_access_tokenslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function newRow()
    {
        $row = [];
        $row['id'] = null;
        $row['tokenable_type'] = null;
        $row['tokenable_id'] = null;
        $row['name'] = null;
        $row['token'] = null;
        $row['abilities'] = null;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpersonal_access_tokenslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
