...
class UserlevelpermissionsAdd extends Userlevelpermissions
{
    ...
    // Page object name
    public $PageObjName = "UserlevelpermissionsAdd";
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->_permission->CurrentValue = null;
        $this->_permission->OldValue = $this->_permission->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'permission' first before field var 'x__permission'
        $val = $CurrentForm->hasValue("permission") ? $CurrentForm->getValue("permission") : $CurrentForm->getValue("x__permission");
        if (!$this->_permission->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->_permission->Visible = false; // Disable update for API request
            } else {
                $this->_permission->setFormValue($val);
            }
        }
    }
    ...
    protected function newRow()
    {
        ...
        $row['permission'] = $this->_permission->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->_permission->EditValue = HtmlEncode($this->_permission->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->_tablename->Required) {
            if (!$this->_tablename->IsDetailKey && EmptyValue($this->_tablename->FormValue)) {
                $this->_tablename->addErrorMessage(str_replace("%s", $this->_tablename->caption(), $this->_tablename->RequiredErrorMessage));
            }
        }
        if ($this->_permission->Required) {
            if (!$this->_permission->IsDetailKey && EmptyValue($this->_permission->FormValue)) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // permission
        $this->_permission->setDbValueDef($rsnew, $this->_permission->CurrentValue, 0, false);
