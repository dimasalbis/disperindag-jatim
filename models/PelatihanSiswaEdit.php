 ...
 * Page class
 */
class PelatihanSiswaEdit extends PelatihanSiswa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PelatihanSiswaEdit";
        ...
        if (IsApi()) {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    $this->terminate("pelatihansiswalist"); // Return to list page
                    return;
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // asal_sekolah
            $this->asal_sekolah->setDbValueDef($rsnew, $this->asal_sekolah->CurrentValue, "", $this->asal_sekolah->ReadOnly);

            // tempat_lahir
            $this->tempat_lahir->setDbValueDef($rsnew, $this->tempat_lahir->CurrentValue, "", $this->tempat_lahir->ReadOnly);

            // tanggal_lahir
            $this->tanggal_lahir->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_lahir->CurrentValue, 0), CurrentDate(), $this->tanggal_lahir->ReadOnly);

            // jenis_kelamin

            // no_telpon
            $this->no_telpon->setDbValueDef($rsnew, $this->no_telpon->CurrentValue, null, $this->no_telpon->ReadOnly);

            // email
            $this->_email->setDbValueDef($rsnew, $this->_email->CurrentValue, null, $this->_email->ReadOnly);

            // legalitas
            if ($this->legalitas->Visible && !$this->legalitas->ReadOnly && !$this->legalitas->Upload->KeepFile) {
                $this->legalitas->Upload->DbValue = $rsold['legalitas']; // Get original value
            ...
            if ($this->legalitas->Visible && !$this->legalitas->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->legalitas->Upload->FileName)) {
                    ...
                    $this->legalitas->setDbValueDef($rsnew, $this->legalitas->Upload->FileName, "", $this->legalitas->ReadOnly);
                }
            }
