 ...
 * Page class
 */
class PasswordResetTokensEdit extends PasswordResetTokens
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PasswordResetTokensEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("_email") ?? Key(0) ?? Route(2)) !== null) {
                $this->_email->setQueryStringValue($keyValue);
                $this->_email->setOldValue($this->_email->QueryStringValue);
            } elseif (Post("_email") !== null) {
                $this->_email->setFormValue(Post("_email"));
                $this->_email->setOldValue($this->_email->FormValue);
            } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->_email->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->_email->CurrentValue, $rs->fields['email'])) {
        ...
        switch ($this->CurrentAction) {
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "passwordresettokenslist") {
