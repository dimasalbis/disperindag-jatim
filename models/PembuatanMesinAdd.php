 ...
 * Page class
 */
class PembuatanMesinAdd extends PembuatanMesin
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PembuatanMesinAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("pembuatanmesinlist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "pembuatanmesinview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->lama_pembuatan->CurrentValue = null;
        $this->lama_pembuatan->OldValue = $this->lama_pembuatan->CurrentValue;
        $this->pemesan->CurrentValue = null;
        $this->pemesan->OldValue = $this->pemesan->CurrentValue;
        $this->alamat->CurrentValue = null;
        $this->alamat->OldValue = $this->alamat->CurrentValue;
        $this->nomor_kontrak->CurrentValue = null;
        $this->nomor_kontrak->OldValue = $this->nomor_kontrak->CurrentValue;
        $this->tanggal_kontrak->CurrentValue = null;
    ...
    protected function newRow()
    {
        ...
        $row['lama_pembuatan'] = $this->lama_pembuatan->CurrentValue;
        $row['pemesan'] = $this->pemesan->CurrentValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        // nama_mesin
        $this->nama_mesin->setDbValueDef($rsnew, $this->nama_mesin->CurrentValue, null, false);
        ...
        // lama_pembuatan
        $this->lama_pembuatan->setDbValueDef($rsnew, $this->lama_pembuatan->CurrentValue, "", false);

        // pemesan
        $this->pemesan->setDbValueDef($rsnew, $this->pemesan->CurrentValue, "", false);

        // alamat
        $this->alamat->setDbValueDef($rsnew, $this->alamat->CurrentValue, "", false);

        // nomor_kontrak
        $this->nomor_kontrak->setDbValueDef($rsnew, $this->nomor_kontrak->CurrentValue, "", false);

        // tanggal_kontrak
        $this->tanggal_kontrak->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_kontrak->CurrentValue, 0), CurrentDate(), false);
        ...
        if ($this->foto_mesin->Visible && !$this->foto_mesin->Upload->KeepFile) {
            $this->foto_mesin->Upload->DbValue = ""; // No need to delete old file
        ...
        if ($this->upload_ktp->Visible && !$this->upload_ktp->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->upload_ktp->Upload->FileName)) {
                ...
                $this->upload_ktp->setDbValueDef($rsnew, $this->upload_ktp->Upload->FileName, "", false);
            }
        }
        if ($this->foto_mesin->Visible && !$this->foto_mesin->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->foto_mesin->Upload->FileName)) {
                ...
                $this->foto_mesin->setDbValueDef($rsnew, $this->foto_mesin->Upload->FileName, "", false);
            }
        }
