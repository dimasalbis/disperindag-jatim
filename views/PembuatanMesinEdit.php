...
// Page object
$PembuatanMesinEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpembuatan_mesinedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fpembuatan_mesinedit = currentForm = new ew.Form("fpembuatan_mesinedit", "edit");
    ...
    fpembuatan_mesinedit.addFields([
    ...
    // Validate form
    fpembuatan_mesinedit.validate = function () {
    ...
    // Form_CustomValidate
    fpembuatan_mesinedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpembuatan_mesinedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpembuatan_mesinedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fpembuatan_mesinedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpembuatan_mesinedit" id="fpembuatan_mesinedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["fpembuatan_mesinedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fpembuatan_mesinedit", "x_tanggal_kontrak", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span></template>
...
<input type="hidden" name="fa_x_foto_kontrak" id= "fa_x_foto_kontrak" value="<?= (Post("fa_x_foto_kontrak") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_upload_ktp" id= "fa_x_upload_ktp" value="<?= (Post("fa_x_upload_ktp") == "0") ? "0" : "1" ?>">
...
<input type="hidden" name="fa_x_foto_mesin" id= "fa_x_foto_mesin" value="<?= (Post("fa_x_foto_mesin") == "0") ? "0" : "1" ?>">
...
<?php } ?>
    ...
    <input type="hidden" data-table="pembuatan_mesin" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<div id="tpd_pembuatan_mesinedit" class="ew-custom-template"></div>
<template id="tpm_pembuatan_mesinedit">
<div id="ct_PembuatanMesinEdit"><?php require_once('list.php'); ?>
</div>
</template>
<?php if (!$Page->IsModal) { ?>
...
loadjs.ready(["jsrender", "makerjs"], function() {
    ...
    ew.applyTemplate("tpd_pembuatan_mesinedit", "tpm_pembuatan_mesinedit", "pembuatan_mesinedit", "<?= $Page->CustomExport ?>", ew.templateData.rows[0]);
